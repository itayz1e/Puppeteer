import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faRecycle } from '@fortawesome/free-solid-svg-icons';
import '../style/scrapeforimages`.scss';

function ScrapeForImages() {
  return (
      <div className="container">
        <div className="container__image">
            <button>
          <img
            src="https://res.cloudinary.com/hjqklbxsu/image/upload/f_auto,fl_lossy,q_auto/v1629127526/product/bottle/d/JD%20US%2010%20Year%20Old%20Tennessee%20Whiskey%20Transparent.png"
            alt=""
            className="image"
            />
            </button>
        </div>
        <div className="container__image">
            <button>
          <img
            src="https://res.cloudinary.com/hjqklbxsu/image/upload/f_auto,fl_lossy,q_auto/v1629127526/product/bottle/d/JD%20US%2010%20Year%20Old%20Tennessee%20Whiskey%20Transparent.png"
            alt=""
            className="image"
            />
            </button>
        </div>
        <div className="container__image">
            <button>
          <img
            src="https://res.cloudinary.com/hjqklbxsu/image/upload/f_auto,fl_lossy,q_auto/v1629127526/product/bottle/d/JD%20US%2010%20Year%20Old%20Tennessee%20Whiskey%20Transparent.png"
            alt=""
            className="image"
            />
            </button>
        </div>
        <button className="RecycleIcon">
        <FontAwesomeIcon icon={faRecycle} className='icon' />
        </button>
        <div>
      <input type="checkbox" className="click" />
      <label htmlFor="bien" className="confirmed">Please select an image and confirm</label>
    </div>
      </div>
  );
}

export default ScrapeForImages;
